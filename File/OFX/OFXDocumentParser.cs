...
namespace OFXSharp
{
        ...
        {
                public OFXDocument Import (FileStream stream, bool isSGMLsansHdr = true)
                {
                        using (var reader = new StreamReader (stream, Encoding.Default))
                        {
                                return Import (reader.ReadToEnd (), isSGMLsansHdr);
                        }
                }

                public OFXDocument Import (string ofx, bool isSGMLsansHdr = true)
                {
                        return ParseOfxDocument (ofx, isSGMLsansHdr);
                }

                private OFXDocument ParseOfxDocument (string ofxString, bool isSGMLsansHdr)
                {
                        //If OFX file in SGML format, convert to XML
                        if (!IsXmlVersion (ofxString) || isSGMLsansHdr)
                        {
                                if (isSGMLsansHdr)
                                {
                                        ofxString = SGMLToXML (AddHeader(ofxString));
                                }
                                else
                                {
                ...
                private string AddHeader (string ofxString)
                {
                        // add a generic header to the file
                        // because is missing
                        StringBuilder sbHeader = new StringBuilder ();
                        sbHeader.AppendLine ("OFXHEADER:100");
                        sbHeader.AppendLine ("DATA:OFXSGML");
                        sbHeader.AppendLine ("VERSION:102");
                        sbHeader.AppendLine ("SECURITY:NONE");
                        sbHeader.AppendLine ("ENCODING:USASCII");
                        sbHeader.AppendLine ("CHARSET:1252");
                        sbHeader.AppendLine ("COMPRESSION:NONE");
                        sbHeader.AppendLine ("OLDFILEUID:NONE");
                        sbHeader.AppendLine ("NEWFILEUID:NONE");
                        StringBuilder sb = new StringBuilder (ofxString);
                        sb.Insert (0, sbHeader);
                        return sb.ToString ();
